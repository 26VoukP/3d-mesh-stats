...
_RECONSTRUCTIONS = flags.DEFINE_string("reconstructions", "",
                                       ...
                                       "to one methods reconstruction of one scene.")
...
def make_plots():
  ...
  for reconstruction in reconstructions:
    folder_path = os.path.join(_INPUT_DIR.value, reconstruction)
    for file_name in os.listdir(folder_path):
      if _GT2PRED_SUFFIX.value in file_name:
    ...
    reconstructions_info.append(ReconstructionInfo(name=reconstruction,
                                                   gt2pred_file=os.path.join(_INPUT_DIR.value, gt2pred_stats),
                                                   pred2gt_file=os.path.join(_INPUT_DIR.value, pred2gt_stats))
                                )

  reconstruction_curves, aggregated_curve = fscore_and_confidence(reconstructions_info,
                                                                  _F_SCORE_PERCENTILE_FROM_MEDIAN.value,
                                                                  "Overall F-Score",
                                                                  _F_SCORE_MAX_DISTANCE_CM.value)

  plot_fscores(reconstruction_curves, _F_SCORE_PERCENTILE_FROM_MEDIAN.value)
  plot_combined_fscores(aggregated_curve, _F_SCORE_PERCENTILE_FROM_MEDIAN.value)
